...
def main():
    ...
    for proc in processes:
        ...
        for f in flavors:
            ...
            print(proc.label)
            samples[proc][f] = Sample(files, "events", f, proc.label)
    ...
    tautag_cfg = deepcopy(ctag_cfg)
    tautag_cfg["sig"] = "tau"
    tautag_cfg["bkg"] = ["c", "g", "q"]
    tautag_cfg["param_plot"].ranges = ((0.9, 1.0), (0.000001, 1.0))

    downtag_cfg = deepcopy(ctag_cfg)
    downtag_cfg["sig"] = "d"
    downtag_cfg["bkg"] = ["u", "g", "s", "c", "b"]
    downtag_cfg["param_plot"].ranges = ((0., 1.0), (0.0001, 1.0))
    
    uptag_cfg = deepcopy(ctag_cfg)
    uptag_cfg["sig"] = "u"
    uptag_cfg["bkg"] = ["d", "g", "s", "c", "b"]
    uptag_cfg["param_plot"].ranges = ((0., 1.0), (0.0001, 1.0))
...
class ROC:
    def __init__(self, name, sample_s, sample_b, param, color, style):
        self.name = "{}_{}".format(name, sample_s.label)
        #self.name = name
        ...
        self.label = "{} vs {} ({})".format(self.sample_s.flavor, self.sample_b.flavor, self.sample_s.label)
...
def dfhs(df, fs, fb, m, nbins, label):
    ...
    #if "7_classes" in score_s and score_s == "recojet_isQ":
    #    score_s == "recojet_isU"
    ...
    print(binary_discr_func)
    df_dict = OrderedDict()
    dh_dict = OrderedDict()

    print("producing roc curve: {} vs {} -- {}".format(fs, fb, label))
    if "7_classes" in label and ("isQ" in score_s or  "isQ"in score_b) :
        df = df.Define("recojet_isQ", "0.5*(recojet_isU + recojet_isD)")
